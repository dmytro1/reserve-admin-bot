...
if( ! class_exists( 'CMB2_Field_Ajax_Search' ) ) {
	...
	class CMB2_Field_Ajax_Search {
		...
		public function setup_admin_scripts() {
			...
			$uri = str_replace( ABSPATH, site_url() . '/', dirname( __FILE__ ) ) . '/';
			wp_register_script( 'jquery-autocomplete-ajax-search', $uri . 'js/jquery.autocomplete.min.js', array( 'jquery' ), self::VERSION, true );
			wp_register_script( 'cmb-ajax-search', $uri . 'js/ajax-search.js', array(
				'jquery',
			...
			wp_enqueue_style( 'cmb-ajax-search', $uri . 'css/ajax-search.css', array(), self::VERSION );
